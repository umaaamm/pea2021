...
class Merchandiselaporan extends MY_Controller {
    ...
    public function index(){
        $data                           = $this->data;
        $menu                           = $this->model_main->get_akses($this->merchandiseLaporan,$data['user_group']);
        $parent_menu                    = $this->model_main->get_akses($this->merchandiseLaporan,$data['user_group']);
        $data['akses']                  = $this->get_akses($data['user_id'],$this->merchandiseLaporan);
        $data['today_date']             = date('d-m-Y');
        $data['menu_id']                = $this->merchandiseLaporan;
        $data['menu_nama']              = $menu->menu_nama;
        $data['menu_controller']        = $this->controller_merchandiseLaporan;
        $data['parent_menu_id']         = $this->merchandiseLaporan;
        $data['parent_menu_nama']       = $parent_menu->menu_nama; 
        $data['parent_menu_controller'] = $this->controller_merchandiseLaporan; 

        $data['title_site']             = $menu->menu_nama; 
        $data['potlet_title']           = 'Laporan Merchandise List';
        ...
        $data['get_data']       = $this->controller_merchandiseLaporan.'/get_data';
        $data['get_group']       = $this->controller_merchandiseLaporan.'/get_group';
        $data['save_data']      = $this->controller_merchandiseLaporan.'/save_data';
        $data['edit_data']      = $this->controller_merchandiseLaporan.'/edit_data';
        $data['delete_data']    = $this->controller_merchandiseLaporan.'/delete_data';
        ...
        if($data['branch_id']!=null){
            $query_data  = array(
                'select'    => '*',
                'from'      => 'tb_merchandise_pea',
                'where_field' => 'id_event_pea = '."'".$data['branch_id']."'");
    
            $data['merchandise_laporan_list'] = $this->model_basic->get_data_join($query_data)->result_array();
        }else{
            $query_data  = array(
                'select'    => '*',
                'from'      => 'tb_merchandise_pea');
        ...
        $this->load->view('merchandiseLaporan');
    ...
    function get_data(){
        ...
        $query_get_data  = array(
            ...
            'where_field' => $this->schema.'tb_merchandise_pea'.'.id_kehadiran = '."'".$id."'"
        );
